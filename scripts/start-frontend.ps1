...
Assert-SonyaLabPortAvailable -Port 5175 -Component "Frontend"
...
$Process = Start-Process -FilePath $Node -ArgumentList $Vite,"--host","0.0.0.0","--port","5175","--strictPort" -WorkingDirectory (Join-Path $Root "frontend") -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $Logs "frontend.out.log") -RedirectStandardError (Join-Path $Logs "frontend.err.log")
